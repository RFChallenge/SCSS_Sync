...
# Parameters for 16QAM
mod_num = 16
mod = QAMModem(mod_num)
...
normalizing_coeff = 1/np.sqrt(10)

def generate_qam16_signal(n_sym=n_sym, mod=mod, oversample_factor=oversample_factor, sPSF=sPSF, Fc=0, Fs=Fs):
...
def modulate_qam16_signal(info_bits, mod=mod, oversample_factor=oversample_factor, sPSF=sPSF, Fc=0, Fs=Fs):
    sB = info_bits
    sQ = mod.modulate(sB)  # Modulated baud points
    sQ_padded = np.zeros(len(sQ)*oversample_factor, dtype=np.complex64)
    start_idx = oversample_factor//2
