...
normalizing_coeff = 2.7044250966666668

default_n_sym = nsc*800
...
def generate_ofdm_signal_Gaussian(n_sym=default_n_sym, coeff=None, mod=mod, nfft=fft_len, cp=cp_len, nsc=nsc):
    assert n_sym%nsc == 0, f'Unable to reshape {n_sym} symbols to fit {nsc} subcarriers'
    if coeff is None:
        coeff = normalizing_coeff
    sB = np.random.randint(2, size=(int(mod.num_bits_symbol*n_sym)))
    sQ = mod.modulate(sB)
    sQ_size = int(np.array(sQ.shape))
    sQ_Gauassian = np.empty(sQ_size, dtype=np.complex128)
    sQ_Gauassian.real = np.random.normal(size=sQ_size)/np.sqrt(2)
    sQ_Gauassian.imag = np.random.normal(size=sQ_size)/np.sqrt(2)
    sQ_Gauassian = sQ_Gauassian.reshape(-1, nsc).T
    y_ofdm_Gauassian = ofdm_tx(sQ_Gauassian, nfft, nsc, cp)
    y_ofdm_Gauassian *= coeff
    return y_ofdm_Gauassian.astype(np.complex64), sQ_Gauassian